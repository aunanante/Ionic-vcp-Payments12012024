...
export class CommerceService {
  ...
  /* async findCommercesByServiceNameWithMonthlyFeePaid(serviceName: string): Promise<any[]> {
    try {
      ...
      for (const commerce of allCommerces || []) {
        ...
        // Check if monthly_fee_paid is true
        if (businessOwner && businessOwner.monthly_fee_paid === true) {
          // Check if services is a string and if any word in it contains the search term (case-insensitive)
          const services: string = commerce.services || '';
  
          if (typeof services === 'string') {
            const serviceWords: string[] = services.toLowerCase().split(/\s+/); // Split services into individual words
  
            // Check if any word contains the search term
            if (serviceWords.some(word => word.includes(serviceName.toLowerCase()))) {
              commercesWithMonthlyFeePaid.push(commerce);
            }
          }
        }
  ...
  /* async findCommercesByServiceNameWithMonthlyFeePaid(serviceName: string): Promise<any[]> {
    try {
      ...
      // Define a mapping of accented characters to non-accented counterparts
      ...
      const removeAccents = (str: string): string =>
        str.replace(/[àáâãäåèéêëìíîïòóôõöùúûüñç]/g, (match) => accentMap[match] || match);
      ...
      for (const commerce of allCommerces || []) {
        ...
        if (businessOwner && businessOwner.monthly_fee_paid === true) {
          ...
          if (typeof services === 'string') {
            ...
            const normalizedServiceName = removeAccents(serviceName);
  
            // Use a regular expression to match words in a case-insensitive manner, allowing hyphens
            const regex = new RegExp(`\\b${normalizedServiceName.replace('-', '\\-')}\\b`, 'i');
  
            // Check if the regex matches any word in the services string
            if (regex.test(removeAccents(services.toLowerCase()))) {
              commercesWithMonthlyFeePaid.push(commerce);
            }
          }
        }
  ...
  /* async findCommercesByServiceNameWithMonthlyFeePaid(serviceName: string): Promise<any[]> {
    try {
      ...
      for (const commerce of allCommerces || []) {
        ...
        // Check if monthly_fee_paid is true
        if (businessOwner && businessOwner.monthly_fee_paid === true) {
          // Check if services is a string and if it contains the search term (case-insensitive)
          const services: string = commerce.services || '';
  
          if (typeof services === 'string') {
            // Adjusted regular expression for partial matching
            const regex = new RegExp(`${serviceName.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}`, 'i');
  
            // Check if the regex matches any part of the services string
            if (regex.test(services.toLowerCase())) {
              commercesWithMonthlyFeePaid.push(commerce);
            }
          }
        }
    ...
    try {
      ...
      for (const commerce of allCommerces || []) {
        ...
        if (businessOwner && businessOwner.monthly_fee_paid === true) {
          ...
          if (typeof services === 'string') {
            // Split the normalized service name into individual words
            const serviceWords: string[] = normalizedServiceName.split(/\s+/);
  
            // Check if all words in the service name are present in the services string
            const allWordsPresent = serviceWords.every(word => services.toLowerCase().includes(word));
